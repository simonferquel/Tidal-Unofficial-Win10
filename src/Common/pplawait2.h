...
namespace std
{
    namespace experimental
    {
        ...
        struct delayed_deleter {
            ...
            static constexpr auto kDelay = std::chrono::milliseconds(100);
            struct item {
                time_point tp;
                void *p;
            };
            ...
            void queue(void* p) {
                ...
                auto should_delete = system_clock::now() - kDelay;
                for (size_t i = 0; i < items.size();) {
                    if (items[i].tp > should_delete) {
                        delete items[i].p;
                        items[i] = items.back();
                ...
                items.push_back({ system_clock::now(), p });
            }

            ~delayed_deleter() {
                // mutex is not really needed unless
                // there are some stray coroutines running
        ...
        __declspec(selectany) delayed_deleter g_delayed_deleter;
#endif
